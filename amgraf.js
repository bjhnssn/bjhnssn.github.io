...
const GRAFFITI = {
	F:[
		...
		'licorice', 'love', 'narcissism', 'piracy', 'politics', 'pride', 'propaganda', 'racism', 'religion', 'sexism', 'stereotypes', 'superheroes', 'uranus', 'vegetables', 'violence', 'vitriol',
		...
		'cold pizza/flat soda', 'stale bagels/weak coffee', 'stale donuts/weak coffee', 'hate/love', 'me/you', 'this/that',
	],
	X:[
...
const colorCombo = [
	...
	// TODO: replace this array w. function for dynamic creation of color combinations
	// TODO: include more tints and shades!

	// -- 1/1/1
	// { bg:B,	b:[ W ],		f:[ R ] },
	// { bg:R,	b:[ W ],		f:[ B ] },
	// { bg:W,	b:[ [0, 0.85, 0.5] ],	f:[ [0.55, 0.75, 0.5] ] },
	...
	// { bg:Y,					b:[ K ],		f:[ Y ] },
	// { bg:B,					b:[ K ],		f:[ B ] },
	// { bg:[orange, 1, 0.5],	b:[ K ],		f:[ [orange, 1, 0.5] ] },
	// { bg:[0.9, 1, 0.75],	b:[ K ],		f:[ Y ] },   // pink
	// { bg:K25,				b:[ K ],		f:[ R ] },

	// gray scales
	// { bg:[0, 0, 0.25],		b:[ [0, 0, 0.125] ],	f:[ [0, 0, 0.875] ] },
	// { bg:[0, 0, 0.875],		b:[ [0, 0, 0.25] ],		f:[ [0, 0, 0.125] ] },
	...
	// -- 1/2/2
	// { bg:K,	b:[ G ],		f:[ [0.9, 1, 0.75], K ] },   // pink
	// { bg:K,	b:[ Y ],		f:[ [0.9, 1, 0.75], K ] },
	// { bg:K,	b:[ R ],		f:[ W, B ] },
	// { bg:[0, 0.9, 0.5],		b:[ [0, 0.9, 0.45], [0, 0.8, 0.45] ],		f:[ [0, 0.8, 0.55], [0, 0.8, 0.45] ]	},   // redish
	// { bg:[0.333, 0.9, 0.5],	b:[ [0.333, 0.9, 0.4], [0.333, 0.8, 0.6] ],	f:[ [0.333, 0.8, 0.55], [0.333, 0.8, 0.45] ] },   // greenish
	// { bg:[0.6, 0.9, 0.4],	b:[ [0.65, 0.9, 0.4], [0.6, 0.8, 0.5] ],	f:[ [0.6, 0.8, 0.4], [0.65, 0.9, 0.5] ] },   // Bish
	...
	// also keep default R/W/B and grayscales as pre-defined combos?
...
function initSeed( _hash ){
		...
		if( type == 'set' ){   // odd
			...
			for( let i=0; i<nColors*2; i++ ){
				...
				if( i<nColors ){
					...
					const adjL = (L != 0.5 ? Math.abs(L-1) : L);   // if L is set to anything but 0.5, change it to the opposite
			...
			for( let i=0; i<nColors+addBg; i++ ){
				...
				if( i<nColors/2 ) ccObj.b.push( [H/255, S, L] );
					...
					const adjL = (L != 0.5 ? Math.abs(L-1) : L);
					ccObj.f.push( [H/255, S, adjL] );
				}
	...
	// clrScheme = getColorScheme( nCC, nPT, nST );
